...
@ActiveProfiles("my-batis")
public class PortfolioMockController {
	
	@Autowired
	MockMvc mockMvc;
	
//	@Qualifier("mainPortfolioService")
//	@MockBean
//	private PortfolioService service;
	...
	void getAllUserPortfolios() throws Exception {
		...
		MvcResult result=mockMvc.perform(get(requestUrl,clientId))
		.andDo(print())
		.andExpect(status().isOk())
		.andReturn();
		
		
		Portfolio[] retrived=new ObjectMapper().readValue(result.getResponse().getContentAsString(), Portfolio[].class);
	...
	public void getAllUserPortfolios_Not_Found() throws Exception {
		...
		when(service.getPortfoliosForAUser(clientId)).thenThrow(NotFoundException.class);
	...
	public void getAllUserPortfolios_Bad_Request() throws Exception {
		...
		when(service.getPortfoliosForAUser(clientId)).thenThrow(DatabaseException.class);
	...
	public void getAllUserPortfolios_Server_Error() throws Exception {
		...
		when(service.getPortfoliosForAUser(clientId)).thenThrow(RuntimeException.class);
	...
	public void getThePortfolioFromItsId() throws Exception {
		...
		when(service.getPortfolioForAuserFromPortfolioId(portfolio2.getPortfolioId())).thenReturn(portfolio2);
...
//	}
	...
	void getThePortfolioFromItsId_Server_Error() throws Exception {
		...
		when(service.getPortfolioForAuserFromPortfolioId(portfolio2.getPortfolioId())).thenThrow(RuntimeException.class);
	...
	void createDefaultPortfolioOfClient() throws Exception {
		...
		when(service.addNewPortfolio(portfolioData)).thenAnswer(new Answer<Portfolio>() {

			@Override
			public Portfolio answer(InvocationOnMock invocation) throws Throwable {
	...
	void createDefaultPortfolioOfClient_Not_Found() throws Exception {
		...
		when(service.addNewPortfolio(portfolioData)).thenThrow(NotFoundException.class);		
	...
	@Test
	void createNewPortfolioOfClient_NotCorrectDataSupplied() throws Exception {
	...
	@Test
	public void deleteThePortfolioFromItsId() throws Exception {
		String requestUrl="/portfolios/{portfolioId}";
...
//	@Test
//	public void deleteThePortfolioFromItsId_Not_Found() throws Exception {
//		String requestUrl="/portfolios/{portfolioId}";
//		
//		doThrow(NotFoundException.class).when(service).deletePortfolioById(portfolio2.getPortfolioId());
//		
//		mockMvc.perform(delete(requestUrl,portfolio2.getPortfolioId()))
//		.andDo(print())
//		.andExpect(status().isNotFound());
...
//	}
	...
	@Test
	public void deleteThePortfolioFromItsId_Bad_Request() throws Exception {
		String requestUrl="/portfolios/{portfolioId}";
		
		doThrow(DatabaseException.class).when(service).deletePortfolioById(portfolio2.getPortfolioId());
	...
	void deleteThePortfolioFromItsId_Server_Error() throws Exception {
		...
		doThrow(RuntimeException.class).when(service).deletePortfolioById(portfolio2.getPortfolioId());
		mockMvc.perform(delete(requestUrl,portfolio2.getPortfolioId()))
		.andDo(print())
		.andExpect(status().is5xxServerError());
